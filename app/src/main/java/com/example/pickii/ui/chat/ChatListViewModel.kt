...
class ChatListViewModel @Inject constructor() : ViewModel() {
     ...
     */
    private fun List<ChatRoomPreviewUiModel>.toggleNotification(
        chatRoomId: Long,
     ...
     * @param chatRoomId 읽음 처리할 채팅방 식별자
     */
    private fun List<ChatRoomPreviewUiModel>.markAsRead(
        chatRoomId: Long,
    ): List<ChatRoomPreviewUiModel> {
        return map { chatRoom ->
            if (chatRoom.id == chatRoomId) {
    ...
    fun removeChatRoom(
        ...
        _uiState.update { currentState ->
            currentState.copy(
                groupChatRooms = currentState.groupChatRooms.filterNot { chatRoom ->
                    chatRoom.id == roomId
                },
                directChatRooms = currentState.directChatRooms.filterNot { chatRoom ->
                    chatRoom.id == roomId
                },
            )
        }
